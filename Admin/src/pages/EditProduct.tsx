...
function categoryIdFromProduct(p: ProductItem): string {
  const c = p.categoryId;
  if (typeof c === "object" && c !== null && "_id" in c) return (c as { _id: string })._id;
...
export default function EditProduct() {
  ...
  useEffect(() => {
    ...
    productsAPI
      ...
      .then((res) => {
        ...
        const p = res.data;
        const id = p.id ?? (p as { _id?: string })._id ?? routeProductId;
        setProductId(id);
        setFormName(p.name);
        setFormDescription(p.description);
        setFormCategoryId(categoryIdFromProduct(p));
        setFormBasePrice(String(p.basePrice));
        setFormRating(String(p.rating ?? 0));
        ...
        setSecondary([
          ...
          { ...emptySlot(), manualUrl: urls[1] ?? "" },
          { ...emptySlot(), manualUrl: urls[2] ?? "" },
        ]);
      })
      .catch((e) => {
        if (!cancelled) {
          setProductLoadError(e instanceof Error ? e.message : "Failed to load product");
        }
      })
      .finally(() => {
  ...
  }, [user, loading, routeProductId, navigate]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    if (!productId) {
      setError("Product is not loaded");
      return;
    }
    if (!formName.trim()) {
  ...
  if (productLoading) {
    return (
      <AdminLayout title="Edit product" userEmail={user?.email}>
  ...
  if (productLoadError || !productId) {
    return (
      <AdminLayout title="Edit product" userEmail={user?.email}>
        <div className="mx-auto max-w-6xl space-y-4">
          <div className="rounded-md bg-destructive/10 border border-destructive/20 px-3 py-2 text-sm text-destructive">
            {productLoadError ?? "Product not found"}
          </div>
          <button
            type="button"
